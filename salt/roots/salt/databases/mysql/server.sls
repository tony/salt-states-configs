{% from "databases/mysql/map.jinja" import mysql with context %}
...
{% if grains['os'] in ['Ubuntu', 'Debian'] %}
    - debconf: salt://databases/mysql/files/mysql.deb.set
{% endif %}
  service:
    - running
    - name: {{ mysql.service }}
...
my.cnf:
  file.managed:
    - name: {{ mysql.config }}
    - source: salt://databases/mysql/files/{{ grains['os'] }}-my.cnf
    - user: root
    - group: root
    - mode: 644
    - template: jinja
...
my.cnf:
  file.managed:
    - name: {{ mysql.config }}
    - source: salt://databases/mysql/files/my-{{ mysql.mysql_size }}.cnf
    - template: jinja
    - watch_in:
      - service: mysqld
{% endif %}
